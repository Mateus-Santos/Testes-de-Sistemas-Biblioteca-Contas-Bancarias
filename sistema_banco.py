...
def Deletar_conta(deletar):
    for item in usuarios:
        if (item[0][1] == deletar[0] and item[0][3] == deletar[1]):
...
def Alterar_limite(alterar):
    for item in usuarios:
        if(item[0][1] == alterar[0] and item[3] == alterar[1]):
...
def Receber_emprestimo(emprestimo):
    for item in usuarios:
        if(item[0][1] == emprestimo[0] and item[3] == emprestimo[1]):
...
def Transferencia_bancaria(transferencia):
    for item in usuarios:
        if(item[0][1] == transferencia[0] and item[3] == transferencia[1]):
            return "Operação feita com sucesso"
    return "Nao foi possivel fazer a operacao"

def Saque(sacar):
    for item in usuarios:
        if(item[0][1] == sacar[0] and item[3] == sacar[1]):
            return "Operação feita com sucesso"
...
def Solicitar_emprestimo(solicitar):
    for item in usuarios:
        if(item[0][1] == solicitar[0] and item[3] == solicitar[1]):
            return "Operação feita com sucesso"
    return "Nao foi possivel fazer a operacao"
...
def Verificar_limite(limite):
    for item in usuarios:
        if(item[0][1] == limite[0] and item[3] == limite[1]):
            return "Operação feita com sucesso"
    return "Nao foi possivel fazer a operacao"
